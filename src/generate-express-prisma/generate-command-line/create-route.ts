...
export const generatorRoute = async (projectPath: string) => {
  ...
  await fs.writeFile(path.join(projectPath, 'src/generator/create-route.ts'), serverCode);
};
...
export const commandCreateTypes = async (routeName: string) => {
  ...
  await fs.writeFile(path.join(__dirname, `${routeNameLower}.types.ts`), serverCode);
};
