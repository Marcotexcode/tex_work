...
export const generatorModel = async (projectPath: string) => {
  const serverCode = `
      import * as fs from "fs/promises";
      import * as path from "path";
      ...
      export const commandCreateModel = async () => {
        ...
        const modelNameUpper =
          modelNameLower.charAt(0).toUpperCase() + modelNameLower.slice(1);
        const fields: Field[] = [];
        ...
        while (true) {
          ...
          const fieldTypeUpper =
            fieldTypeLower.charAt(0).toUpperCase() + fieldTypeLower.slice(1);
  ...
  await fs.writeFile(path.join(projectPath, 'src/generator/create-model.ts'), serverCode);
};
